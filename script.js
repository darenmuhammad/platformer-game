...
// below this is how to get child element
...
class Platform {
    constructor(x, y) {
        this.position = { x, y }; // shorthand property name syntax
        this.width = 200;
        this.height = proportionalSize(40);
    }
...
const animate = () => {
    ...
    platforms.forEach((platform) => {
        // array can be use to store boolean expression
    ...
    checkpoints.forEach((checkpoint, index, checkpoints) => {
        const checkpointDetectionRules = [
            ...
            // the rule below will ensure that the player is close enough to the checkpoint to claim it
            player.position.x - player.width <= checkpoint.position.x - checkpoint.width + player.width * 0.9,
            // the rule below will ensure that the player can only claim the first checkpoint or a checkpoint that has already been claimed
